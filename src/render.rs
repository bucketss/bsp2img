...
impl Renderer {
    ...
    pub fn draw_crop(
        ...
        let Some(pt) = t.post.as_ref().filter(|_| crate::post::needed(look)) else {
            self.encode_world(enc, &t.color_view, None, &t.depth_view, view, &m, aspect, sky_crop, cuts, look.cull, clear, anim, time);
            return;
        };
        let scene = pt.scene();
        self.encode_world(enc, scene, Some(&pt.normal), &t.depth_view, view, &m, aspect, sky_crop, cuts, look.cull, clear, anim, time);
